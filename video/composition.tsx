...
function Board({ scene, icons, index, fps }: { scene: TimedScene; icons: Record<string, string>; index: number; fps: number }) {
  const frame = useCurrentFrame();
  ...
  return <AbsoluteFill style={{ background: "#fbfaf5", color: ink, fontFamily: "Arial, sans-serif", padding: "52px 72px" }}>
    ...
    <div style={{ position: "absolute", right: 72, top: 55, fontSize: 16 }}>{index + 1} / 3</div>
    ...
    <div style={{ position: "absolute", bottom: 17, right: 35, fontSize: 12, color: "#66745f" }}>Original scripted renderer demo · Kokoro narration</div>
    <Sequence from={8}><Audio src={staticFile(scene.audioFile)} /></Sequence>
  </AbsoluteFill>;
}

export function ExplainerVideo({ project, icons }: VideoProps) {
  return <AbsoluteFill>{project.scenes.map((scene, index) => <Sequence key={scene.id} from={scene.startFrame} durationInFrames={scene.durationInFrames}><Board scene={scene} icons={icons} index={index} fps={project.fps} /></Sequence>)}</AbsoluteFill>;
}
