...
Published entry conditions: a new app begun on/after **August 25, 2026, noon PT**; Convex backend; public GitHub repository and working `convex.site`/`chatgpt.site` frontend; root `hackathon.md`; demo below three minutes. Submit through the organizer-linked [VibeApps form](https://vibeapps.dev/judging/convex-all-gas-hackathon-openai/submit) by **September 22, noon PT**. The sponsor criteria expect actual OpenAI, Firecrawl, and AgentMail product functions. They do not separately define a minimum pass/fail threshold per sponsor; this plan implements all three rather than assuming Codex usage alone is sufficient. [Official rules and judging criteria](https://www.convex.dev/hackathons/all-gas).
...
| Sponsor functions | Firecrawl already planned; OpenAI and AgentMail absent. | Add scene review and requested result delivery as described in Section 4.6. |
...
| Visual critique | Qualified OpenAI image-input model through Responses API | Reviews actual sampled scene frames with script and claims; reports structured defects. Geometry checks always run. |
...
Convex database + Workflow + File Storage + vector search
    ...
    ├─ actions → OpenAI rendered-frame review
...
| OpenAI visual-review role | Pin an account-accessible image-input model after the first real-call qualification | Verify image inputs, structured defects, latency and spend in Section 4.6; model selection is a setup gate. |
| Post-hackathon alternate visual critic | `@cf/meta/llama-4-scout-17b-16e-instruct` | Qualify image payloads and defect reports before admitting it. Do not treat this route as proof of OpenAI product use. [Model page](https://developers.cloudflare.com/workers-ai/models/llama-4-scout-17b-16e-instruct/) |
...
**OpenAI visual reviewer.** After deterministic preview checks pass, send actual rendered images, scene IDs/times, narration excerpts and the claim ledger to an image-capable OpenAI model through the Responses API. Start with each board's final frame and selected reveal-state frames, preserving enough resolution for labels. Use a supported JSON schema for `CriticReport`; validate scene/frame references and output independently. Pin the chosen model after testing a clean scene and injected clipping, wrong-arrow and missing-entity cases. Image inputs and structured outputs are documented capabilities; small text, counting and spatial judgments remain imperfect. [Image inputs and limitations](https://developers.openai.com/api/docs/guides/images-vision), [structured outputs](https://developers.openai.com/api/docs/guides/structured-outputs).
...
For the hackathon profile, an unavailable OpenAI reviewer leaves visual QA pending until the job deadline, then returns an explicit failure with the draft. Keep NVIDIA/Cloudflare routing for text work. A later qualified alternate critic can improve availability, but its output must not be presented as an OpenAI review. Configure an OpenAI API budget separately from the coding-agent subscription; verify account access during H0.
...
- OpenAI reviews actual frames and its result affects publication or repair. Firecrawl retrieves evidence actually used by a script. One requested AgentMail delivery is proven with provider IDs and receipt in a consented test inbox. Redact private identifiers in public evidence.
...
| OpenAI critic unavailable in the hackathon profile | Bound the waiting period and retain a draft; do not silently attribute another provider's review to OpenAI. |
...
Use actual NVIDIA/Cloudflare/OpenAI/Firecrawl/AgentMail usage and a dated price table. Track OpenAI image-input tokens separately from ordinary text estimates. Local Kokoro has no hosted TTS per-character fee, but CPU time, memory, setup, storage and electricity remain real costs. If a provider does not report usage, record the estimate and its basis. Separate marginal generation cost from the one-time illustration-library and engineering investment. No dollars-per-video or minutes-to-video claim is established by this review.

Use the user-provided Zerops credits for media compute. Set development and public-demo spending caps in H0, including a reserve for sponsor API calls and judging-period generation. Do not budget against unverified promotional credits or assume that access to Codex includes an OpenAI API balance. Record activated credits separately from expected credits, and establish the deployed plan's storage/transfer allowance before inviting users.
...
| Sponsor integration is only decorative | An installed SDK or static badge does not demonstrate a functioning feature. | OpenAI review affects output; Firecrawl evidence supports claims; AgentMail sends a requested working lesson link. |
...
- [OpenAI image inputs](https://developers.openai.com/api/docs/guides/images-vision) and [structured outputs](https://developers.openai.com/api/docs/guides/structured-outputs): rendered-frame critic interface.
