...
async function heartbeat() {
  try {
    const response = await fetch(new URL("/api/worker/heartbeat", url), { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ workerId, instanceId, version: "0.5.2", capabilities: ["kokoro", "remotion", "fixture-v1", "generated-v1", "review-frames-v1", "explicit-connections-v1", "text-cards-v1"] }), signal: AbortSignal.timeout(10_000) });
...
async function poll() {
  ...
  try {
    const task = await api<{ taskId: string; attempt: number; fixtureVersion: string; projectJson?: string; provenanceJson?: string } | null>({ op: "claim", worker, protocol: 5 });
    if (!task) return;
...
const server = createServer((request, response) => {
  ...
  response.end(JSON.stringify({ ready, phase: "review-and-delivery", capabilities: ["kokoro", "remotion", "fixture-v1", "generated-v1", "review-frames-v1", "explicit-connections-v1", "text-cards-v1"], busy: inFlight }));
});
await heartbeat();
