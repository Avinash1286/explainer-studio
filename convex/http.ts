...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    ...
    if (typeof data.workerId !== "string" || !/^[a-zA-Z0-9_-]{1,80}$/.test(data.workerId)
      ...
      || !Array.isArray(data.capabilities) || data.capabilities.length > 9
...
const mediaRequest = z.discriminatedUnion("op", [
  z.object({ op: z.literal("claim"), worker: lease.worker, protocol: z.union([z.literal(2), z.literal(3), z.literal(4), z.literal(5), z.literal(6), z.literal(7)]).optional() }).strict(),
