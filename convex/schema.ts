...
export default defineSchema({
  ...
  lessonReviews: defineTable({ jobId: v.id("jobs"), revision: v.number(), status: v.union(v.literal("pending"), v.literal("passed"), v.literal("rejected"), v.literal("unavailable")), reportJson: v.optional(v.string()), model: v.optional(v.string()), responseId: v.optional(v.string()), usageJson: v.optional(v.string()), createdAt: v.number() }).index("by_jobId_and_revision", ["jobId", "revision"]),
  ...
  revisionRequests: defineTable({ jobId: v.id("jobs"), fromRevision: v.number(), requestId: v.string(), sceneIds: v.array(v.string()), instruction: v.string(), status: v.union(v.literal("pending"), v.literal("completed"), v.literal("failed")), automatic: v.boolean() }).index("by_jobId_and_requestId", ["jobId", "requestId"]),
