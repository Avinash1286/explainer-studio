...
export const planScenes = internalAction({ args: { jobId: v.id("jobs") }, handler: async (ctx, { jobId }): Promise<null> => {
  ...
  const result = await structured(providerConfig(), SYSTEM, prompt, schema, value => validateDraft(alignDraftCues(input.schema.parse(value)), sources, job.duration), fetch, "nvidia", { fallbackOnInvalid: true });
