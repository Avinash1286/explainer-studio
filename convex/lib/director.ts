...
const styleExample = {
  ...
  visualPlan: {
    ...
    entities: [
      ...
      { id: "valve", kind: "valve", label: "Valve", x: 44, y: 42, w: 32, h: 38, color: "ink", enter: 0.1, cue: "closed valve", variant: "closed" },
...
export function validateDirectedPlan(value: unknown, narration: string): VisualPlan {
  ...
  if (!primary.some(e => e.w >= 24 && e.h >= 22)) errors.push("Give the scene a clearly visible focal component at least 24% wide and 22% high; keep smaller objects as context rather than a row of tiny icons.");
...
export function directorInput(project: Project, sources: Research, sceneId: string, instruction = "") {
  ...
  const prompt = JSON.stringify({
    ...
    direction: [
      ...
      "x/y are absolute percentage centers on the full 16:9 canvas, w/h are percentage dimensions. Design one dominant meaningful component at least 24% wide and 22% high, with smaller context around it. Labeled primary objects need at least 12% width and height; only particles or contained components may be smaller. An atom or valve is not a tiny 4% icon at the edge of a vast empty canvas. Keep bounds in x 3..97, y 4..95 and allow 5 extra vertical units below labeled objects. Separate starting bounds: overlaps exceeding 22% of the smaller object require intentional parentId containment. Keep labels apart and comparable objects at consistent scale. Parented photons/electrons/tokens under 12% height MUST have label empty; their fixed-size text would hide the material. Explain them with a nearby readable annotation or the material's context label. Labels are 2–3 word annotations, not narration subtitles. Do not create persistent titles, headers, footers, takeaway banners or scene counters.",
  ...
  return { schema, prompt, validate: (value: unknown) => validateDirectedPlan(value, scene.narration) };
}

export type DirectorAttempt = { sceneId: string; attempts: Attempt[] };
export async function directScenes(config: ProviderConfig, project: Project, sources: Research, sceneIds = project.scenes.map(s => s.id), instruction = "", transport: typeof fetch = fetch) {
  ...
  for (let i = 0; i < sceneIds.length; i += 2) {
    const results = await Promise.all(sceneIds.slice(i, i + 2).map(async sceneId => {
      const input = directorInput(directed, sources, sceneId, instruction);
