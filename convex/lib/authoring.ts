...
export function authoringInput(sources: Research, duration: number, topic: string, audience: string) {
  ...
  const prompt = JSON.stringify({
    ...
    diagramRules: "After writing each narration, select 2 or 3 distinct concrete nouns or technical noun phrases from that narration for its icons array. These are natural-language concept labels of 2-24 characters, NOT stock-image IDs. Every word in each label must occur in the core narration. The renderer will choose a literal illustration when one exists and an animated word card otherwise; ALL concepts are allowed, so never introduce a gear, leaf, sun, cloud, thermometer or other object merely to obtain an illustration. Avoid lone verbs, adverbs or prepositions as concept labels. Prefer connections:[] unless an arrow actually clarifies the mechanism. If an arrow is necessary, its label and endpoints must form a true complete relationship, including needed prepositions: water vapor -> droplets, label 'condenses into'. Never transfer the action to a container, imply an unsupported transformation, or add an arrow just for decoration.",
