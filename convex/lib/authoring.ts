...
export function authoringInput(sources: Research, duration: number, topic: string, audience: string) {
  ...
  const prompt = JSON.stringify({
    ...
    causalStory: "For each mechanism preserve the source's necessary intermediate steps: the starting state, the interaction, and the resulting changed state. Name the affected object and what changes, so it can be shown rather than represented by a floating word or decorative highlight. Do not collapse a chain into a false direct cause. Include a return path when the source requires a circuit, circulation or feedback; do not invent a loop for a one-way process. Use successive scenes for genuinely different views or stages, such as context, an enlarged interior mechanism, and its consequence, rather than repeating the same exterior-object diagram.",
