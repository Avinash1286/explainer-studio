...
export async function generationReady(ctx: QueryCtx, operatorCanary = false) {
  ...
  return configured && (operatorCanary || env.GENERATION_ENABLED === "true") && qualification?.passed === true && icons.length === manifest.entries.length;
}
